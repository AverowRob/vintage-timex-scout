...
| D3 | Etsy as the second marketplace (next) | Source (E1) | Next | The adapter is already wired and emits the same `Listing`; it only awaits Etsy's own API approval, so it is the nearest expansion — Next, not Later. We still prove the end-to-end workflow on one high-volume platform (eBay) first. Etsy demonstrably sells vintage Timex (the brief's third example is an Etsy listing, and a native under-$50 vintage filter exists). |
...
| D6 | Shipping-unknown → include + flag | Filter (E2) | MVP | When the API returns no shipping quote, the item is kept (gated on item price) and flagged "shipping not listed" rather than dropped — favor recall (D16). In the MVP now that the gate is on landed cost (D28). |
...
| D28 | Gate on **total landed cost** (item + shipping to M6K1V8 ≤ $50), per the brief | Filter (E2) | MVP | The brief's budget is "under $50 *including shipping*", so the gate is on **total cost, not item price**. Shipping is read from the Browse API's `shippingOptions` (requested for M6K1V8 via the end-user-context header), so it needs no extra calls; free shipping is $0; an unquoted item is kept on item price and flagged (D6, favor recall). The source-side filter still caps *item* price at $50 — a valid superset, since shipping ≥ 0 means an item over $50 can never land at ≤ $50. *(This reverses the earlier plan to defer landed cost — it was a core brief requirement, not a secondary signal.)* FX normalization for non-CAD sources remains deferred (D19). |
