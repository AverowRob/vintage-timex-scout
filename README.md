...
| **E2. Filter / gate** | Total landed cost (item + shipping to M6K1V8) ≤ $50, pulled in because the eBay API exposes shipping (D28); exclude explicitly broken. Single source is ebay.ca, so item + shipping are already CAD (no conversion needed) | adjustable price cap; broaden-location toggle | FX to CAD, only once a non-CAD source is added (e.g. ebay.com in USD, Etsy) |
...
| D9 | Carrier-rate shipping estimator (for listings eBay won't quote) | Shipping (E9) | MVP (provided shipping) / Later (estimator) | Shipping is now in the budget: the gate uses eBay's own shipping quotes to M6K1V8 for landed cost (D28), and any unquoted listing is kept on item price and flagged (D6). What stays Later is computing shipping ourselves when eBay returns no quote: low dollar stakes at this price point, and it needs a carrier API plus seller-location parsing. |
...
| D11 | Lightweight file + in-memory state in the MVP; full datastore deferred | Architecture | Next (enabler) | "Memory" here is the running Python process plus two small files in `state/`: the taste brief (`taste.md`) and the keyword profile (`taste_profile.json`), not a database. On a machine with a stable disk (a laptop) the brief survives restarts; on a free ephemeral host (Render) the disk resets on spin-down, so live edits revert to the seeded brief and only the committed listings snapshot is guaranteed. A real datastore (durable cross-session and multi-user state, plus the new-vs-seen E6 and awareness E8 features) is the deferred enabler. |
